...
const UpdateMenuItem = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validate()) {
      ...
      try {
        const response = await axios.put(`${import.meta.env.VITE_SERVER_URL}/menu-items/admin/${id}`, newFormData, {
          headers: {
  ...
  if (isFetching) {
    return <p className="text-sm text-gray-500">Loading menu item...</p>;
  }

  return (
    <div className="max-w-md">
      <h1 className="text-xl font-semibold text-gray-900 mb-4">Edit menu item</h1>
      <form
